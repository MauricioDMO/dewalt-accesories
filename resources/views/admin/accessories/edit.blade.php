...
@section('title', 'Editar Accesorio - Admin')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900">Editar Accesorio</h1>
    <p class="text-gray-600 mt-1">{{ $accessory->code }} - {{ $accessory->name }}</p>
</div>
    ...
    <form action="{{ route('admin.accessories.update', $accessory) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="code" id="code" value="{{ old('code', $accessory->code) }}" required
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $accessory->name) }}" required
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price" id="price" value="{{ old('price', $accessory->price) }}" required
            ...
            <div>
                ...
                <input type="number" step="0.01" name="offer" id="offer" value="{{ old('offer', $accessory->offer) }}"
                ...
                @error('offer')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-sm text-gray-500">Deja vacío si no hay oferta</p>
            </div>

            <!-- Tipo -->
            <div>
                <label for="type" class="block text-sm font-medium text-gray-700">Tipo</label>
                <input type="text" name="type" id="type" value="{{ old('type', $accessory->type) }}"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
            </div>

            <!-- Units -->
            <div class="flex items-center">
                <input type="checkbox" name="units" id="units" value="1" {{ old('units', $accessory->units) ? 'checked' : '' }}
            ...
            <div>
                ...
                <select name="category_id" id="category_id"
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('category_id', $accessory->category_id) == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="subcategory_id" id="subcategory_id"
                    ...
                    @foreach($subcategories as $subcategory)
                        <option value="{{ $subcategory->id }}" {{ old('subcategory_id', $accessory->subcategory_id) == $subcategory->id ? 'selected' : '' }}>
                            {{ $subcategory->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="image" id="image" value="{{ old('image', $accessory->image) }}"
                ...
                @if($accessory->image)
                    <div class="mt-2">
                        <img src="{{ $accessory->image_url }}" alt="{{ $accessory->alt }}" class="h-20 rounded">
                    </div>
                @endif
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="alt" id="alt" value="{{ old('alt', $accessory->alt) }}"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="4"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-yellow-500 focus:ring-yellow-500">{{ old('description', $accessory->description) }}</textarea>
            </div>
        </div>
...
<script>
    ...
    document.getElementById('category_id').addEventListener('change', function() {
        ...
        const currentSubcategoryId = {{ $accessory->subcategory_id ?? 'null' }};
