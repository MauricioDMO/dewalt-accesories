    ...
    <form action="{{ route('admin.categories.update', $category) }}" method="POST">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded p-4 mb-6">
            ...
            <ul class="text-sm text-gray-600 space-y-1">
                <li>• Esta categoría tiene <strong>{{ $category->subcategories()->count() }}</strong> subcategoría(s)</li>
                <li>• Esta categoría tiene <strong>{{ $category->accessories()->count() }}</strong> accesorio(s) asociado(s)</li>
            </ul>
        </div>
